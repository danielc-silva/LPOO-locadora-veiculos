...
class AlugadoState(VeiculoState):
    def alugar(self):
        raise ValueError (f"Reserva Negada. O veículo {self.veiculo.placa} já está sob locação ativa de outro cliente.")
    ...
    def enviar_manutencao(self):
        print("O carro está na rua com um cliente, impossível fazer manutenção agora.")


class ManutencaoState(VeiculoState):
    def alugar(self):
        raise ValueError (f"O veículo {self.veiculo.placa} não está apto à rodagem.")
    ...
    def enviar_manutencao(self):
        print("O veículo já se encontra na oficina no momento.")